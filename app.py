...
def parse_trifecta_payout(html, combo):
    """
    公式結果ページから3連単の100円あたり払戻金を取得する。
    HTML構造変更に強いよう、表→行→ページ文字列の順で探す。
    """
    target_combo = combo.replace("－", "-").replace(" ", "")
    ...
    try:
        ...
        for table in tables:
            for _, row in table.iterrows():
                ...
                if (
                    ("3連単" in row_text or "三連単" in row_text)
                    and target_combo.replace("-", "")
...
def update_pending_results(limit=100):
    ...
    for doc in docs:
        ...
        payout_bug_candidate = (
            data.get("hit") is True
            and int(data.get("payout") or 0) == 0
        )

        if not needs_check and not payout_bug_candidate:
            continue
