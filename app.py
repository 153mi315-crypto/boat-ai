...
def refresh_due_shadow_predictions(now=None, min_minutes_before=0.5, max_minutes_before=2.5, max_races=8):
    now = now or datetime.now(JST)
    target_date = now.date()
    venue_codes = fetch_active_venue_codes(target_date)
    due_races = []
        ...
        try:
            ...
            for race in deadlines:
                ...
                if min_minutes_before <= minutes_before <= max_minutes_before:
                    due_races.append({**race, "minutes_before": round(minutes_before, 2)})
        except Exception as exc:
            schedule_errors.append({
                "venue": VENUES.get(venue_code, str(venue_code)),
    ...
    for race in due_races:
        ...
        if existing_shadow.get("near_deadline_refresh") is True:
            skipped.append({"race": race_text, "reason": "already_refreshed"})
            continue
        ...
        try:
            ...
            doc_ref.update({
                ...
                "shadow_fetched_at": datetime.now(JST),
                "shadow_minutes_before_deadline": race["minutes_before"],
            })
            updated.append({
                "race": race_text,
                "deadline": race["deadline"].isoformat(),
                "minutes_before": race["minutes_before"],
                "status": "ok",
            })
        except Exception as exc:
    ...
    return {
        "status": "ok",
        "checked_at": now.isoformat(),
        "window_minutes": [min_minutes_before, max_minutes_before],
        "due_races": len(due_races),
        "updated": updated,
        "skipped": skipped,
        "schedule_errors": schedule_errors[:10],
