...
def detect_race_grade(html):
    ...
    if re.search(r"(^|[^A-Z0-9])SG([^A-Z0-9]|$)", page_text):
        return "SG"

    if (
        "GⅠ" in page_text
        or "ＧⅠ" in page_text
        or re.search(r"(^|[^A-Z0-9])G1([^A-Z0-9]|$)", page_text)
        or "G1" in compact
    ):
        return "G1"
    ...
    if (
        "GⅡ" in page_text
        or "ＧⅡ" in page_text
        or re.search(r"(^|[^A-Z0-9])G2([^A-Z0-9]|$)", page_text)
        or "G2" in compact
    ):
        return "G2"
    ...
    if (
        "GⅢ" in page_text
        or "ＧⅢ" in page_text
        or re.search(r"(^|[^A-Z0-9])G3([^A-Z0-9]|$)", page_text)
        or "G3" in compact
    ):
        return "G3"
...
def update_pending_results(limit=100):
    ...
    for doc in docs:
        ...
        needs_grade_backfill = not data.get("race_grade")
