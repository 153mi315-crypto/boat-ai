...
def load_shadow_assets():
    ...
    required = [
        SHADOW_FIRST_MODEL_PATH,
        SHADOW_SECOND_MODEL_PATH,
        SHADOW_THIRD_MODEL_PATH,
        SHADOW_SCHEMA_PATH,
    ]
    ...
    try:
        with open(SHADOW_SCHEMA_PATH, "r", encoding="utf-8") as f:
            schema = json.load(f)
        _shadow_loaded = {
            "first_model": joblib.load(SHADOW_FIRST_MODEL_PATH),
            "second_model": joblib.load(SHADOW_SECOND_MODEL_PATH),
            "third_model": joblib.load(SHADOW_THIRD_MODEL_PATH),
            "schema": schema,
        }
        return _shadow_loaded
    except Exception:
...
def predict_combo_table(race_source, assets):
    ...
    for first_lane in range(1, 7):
        ...
        for _, second in sec.iterrows():
            ...
            third = race[
                ~race["lane"].isin([first_lane, second_lane])
            ].copy()
...
def predict_shadow_only(race_text: str, date_text: str | None = None):
    ...
    return {
        ...
        "all_combo_predictions": [
            {
                "combo": row.combo,
                "probability": round(float(row.probability), 8),
                "odds": None if pd.isna(row.odds) else round(float(row.odds), 1),
                "EV": None if pd.isna(row.EV) else round(float(row.EV), 4),
                "rank": int(rank),
            }
            for rank, row in enumerate(shadow_out.itertuples(), start=1)
        ],
    }


def refresh_due_shadow_predictions(
    now=None,
    min_minutes_before=0.05,
    max_minutes_before=2.5,
    max_races=12,
):
    now = now or datetime.now(JST)
    target_date = now.date()
    ...
    for race in due_races:
        ...
        if not snap.exists:
            skipped.append({
                "race": race_text,
                "reason": "base_prediction_missing",
            })
            continue

        existing = snap.to_dict() or {}
        existing_shadow = (
            existing.get("shadow_model")
            or {}
        )

        previous_value = (
            existing_shadow.get("fetched_at")
            or existing.get("shadow_fetched_at")
        )

        try:
            previous_dt = parse_saved_datetime(
                previous_value
            )
        except Exception:
            previous_dt = None
        ...
        if previous_dt is not None:
            ...
            if elapsed_seconds < 45:
                skipped.append({
                    "race": race_text,
                    "reason": "refreshed_within_45_seconds",
        ...
        try:
            shadow_result = predict_shadow_only(
                race_text,
                target_date.isoformat(),
            )
            ...
            snapshots = existing.get(
                "shadow_odds_snapshots"
            )
            if not isinstance(snapshots, list):
                snapshots = []

            snapshot = {
                "fetched_at": shadow_result.get(
                    "fetched_at"
                ),
                "minutes_before_deadline": race[
                    "minutes_before"
                ],
                "all_combo_predictions": shadow_result.get(
                    "all_combo_predictions",
                    [],
                ),
            }
            ...
            doc_ref.set({
                ...
                "shadow_fetched_at": saved_at,
                "shadow_minutes_before_deadline": race[
                    "minutes_before"
                ],
                "shadow_odds_snapshots": snapshots,
                "shadow_refresh_count": len(snapshots),
            }, merge=True)
...
def normalize_grade_token(value):
    ...
    if re.search(r"(^|[^A-Z0-9])SG([^A-Z0-9]|$)", value):
        return "SG"
    if re.search(r"(^|[^A-Z0-9])G1([^A-Z0-9]|$)", value):
        return "G1"
    if re.search(r"(^|[^A-Z0-9])G2([^A-Z0-9]|$)", value):
        return "G2"
    if re.search(r"(^|[^A-Z0-9])G3([^A-Z0-9]|$)", value):
        return "G3"
    if "一般" in value:
        return "一般"
...
def grade_from_schedule_block(block):
    ...
    for pattern, grade in [
        (r"(?:GRADE|ICON|LABEL)[_\-/ ]*SG", "SG"),
        (r"(?:GRADE|ICON|LABEL)[_\-/ ]*G1", "G1"),
        (r"(?:GRADE|ICON|LABEL)[_\-/ ]*G2", "G2"),
        (r"(?:GRADE|ICON|LABEL)[_\-/ ]*G3", "G3"),
    ]:
...
def fetch_official_schedule_grade(target_date, venue_code):
    ...
    # 会場名を含む要素から、開催一覧を含みそうな親要素を探す
    for node in soup.find_all(
        string=lambda s: (
            s is not None
            and venue_compact in compact(s)
        )
    ):
        current = node.parent
    ...
    for venue_block in candidates:
        # 開催単位と思われる子要素を幅広く確認
        event_blocks = venue_block.find_all(
            ["li", "tr", "article", "section", "div"],
            recursive=True,
        )
        ...
        for event_block in event_blocks:
            ...
            if not target_date_in_range(
                target_date,
                *match.groups(),
            ):
                continue

            grade = grade_from_schedule_block(
                event_block
            )
    ...
    # HTML構造変更時のフォールバック:
    # 会場名の周辺HTMLから対象期間とグレードを探す
    raw_html = response.text
    venue_positions = [
        match.start()
        for match in re.finditer(
            re.escape(venue_name),
            raw_html,
        )
    ]
    ...
    for position in venue_positions:
        ...
        for block in fragment_soup.find_all(
            ["li", "tr", "article", "section", "div"]
        ):
...
def detect_race_grade(html):
    ...
    if (
        ...
        or "GRADE_G3" in compact
        or "ICON_G3" in compact
    ):
        return "G3"
    ...
    if (
        ...
        or "GRADE_G2" in compact
        or "ICON_G2" in compact
    ):
        return "G2"
    ...
    if (
        ...
        or "GRADE_G1" in compact
        or "ICON_G1" in compact
    ):
        return "G1"
    ...
    if (
        ...
        or "GRADE_SG" in compact
        or "ICON_SG" in compact
    ):
        return "SG"
...
def predict_internal(race_text: str, date_text: str | None = None):
    ...
    try:
        ...
        if shadow_assets is None:
            ...
            shadow_out["EV"] = (
                shadow_out["probability"] * shadow_out["odds"]
            )
            shadow_out = shadow_out.sort_values(
                ["probability", "EV"],
                ascending=False,
                ...
                "all_combo_predictions": [
                    {
                        ...
                        "probability": round(float(row.probability), 8),
                        "odds": (
                            None if pd.isna(row.odds)
                            else round(float(row.odds), 1)
                        ),
                        "EV": (
                            None if pd.isna(row.EV)
                            else round(float(row.EV), 4)
                        ),
                        "rank": int(rank),
                    }
                    for rank, row in enumerate(
                        shadow_out.itertuples(),
                        start=1,
                    )
                ],
            }
        ...
        shadow_result = {
            "status": "error",
            "model_version": SHADOW_MODEL_VERSION,
            "error": f"{type(shadow_error).__name__}: {shadow_error}",
            "all_combo_predictions": [],
        }

    combined_race_html = racelist_html + before_html
    race_grade = resolve_race_grade(
        target_date,
        venue_code,
        combined_race_html,
    )
...
def fetch_official_result(date_text, venue_code, race_no):
    ...
    # 結果ページだけでは大会グレードが載らない場合があるため、
    # 同日の出走表ページも合わせて判定する。
    try:
        racelist_html, _ = get_html(
            "racelist",
            hd,
            int(venue_code),
            int(race_no),
    ...
    grade = resolve_race_grade(
        result_date,
        int(venue_code),
        result_html + racelist_html,
    )
...
def update_pending_results(limit=100):
    ...
    for doc in docs:
        ...
        try:
            ...
            shadow_predictions = (
                shadow_model.get("all_combo_predictions", []) or []
            )
            shadow_result_prediction = next(
                (
                    item
                    for item in shadow_predictions
                    if str(item.get("combo", "")) == combo
                ),
                None,
            )
            ...
            if shadow_result_prediction:
                shadow_result_probability = (
                    shadow_result_prediction.get("probability")
                )
            ...
            doc.reference.update({
                ...
                "shadow_result_probability": shadow_result_probability,
                "shadow_result_odds": shadow_result_odds,
                "shadow_result_ev": shadow_result_ev,
                "shadow_result_rank": shadow_result_rank,
                "shadow_top1_hit": shadow_top1_hit,
                "shadow_top3_hit": shadow_top3_hit,
                "shadow_top5_hit": shadow_top5_hit,
                "shadow_top10_hit": shadow_top10_hit,
                "shadow_top20_hit": shadow_top20_hit,
                "shadow_model_version": shadow_model.get(
                    "model_version",
                    SHADOW_MODEL_VERSION,
                ),
                "shadow_model_status": shadow_model.get(
                    "status",
                    "unavailable",
                ),
...
def build_stats():
    ...
    comparable_rows = [
        row
        for row in rows
        if row.get("result_checked") is True
        and row.get("result_predicted_rank") is not None
        and row.get("shadow_result_rank") is not None
        and row.get("shadow_model_status") == "ok"
    ]
    ...
    comparison = {
        "races": len(comparable_rows),
        "old": {
            "top1": topn_rate(comparable_rows, "result_predicted_rank", 1),
            "top3": topn_rate(comparable_rows, "result_predicted_rank", 3),
            "top5": topn_rate(comparable_rows, "result_predicted_rank", 5),
            "top10": topn_rate(comparable_rows, "result_predicted_rank", 10),
            "top20": topn_rate(comparable_rows, "result_predicted_rank", 20),
        },
        "new": {
            "top1": topn_rate(comparable_rows, "shadow_result_rank", 1),
            "top3": topn_rate(comparable_rows, "shadow_result_rank", 3),
            "top5": topn_rate(comparable_rows, "shadow_result_rank", 5),
            "top10": topn_rate(comparable_rows, "shadow_result_rank", 10),
            "top20": topn_rate(comparable_rows, "shadow_result_rank", 20),
        },
    }
    ...
    def virtual_roi(
        ...
        return {
            ...
            "roi": total_payout / total_stake if total_stake else None,
            "insufficient_snapshot_races": insufficient_snapshot_races,
            "unstable_picks_excluded": unstable_picks_excluded,
            "safe_odds_rule": "latest_two_lower_odds",
            "max_odds_ratio": 2.0,
        }

    comparison["virtual_rule"] = {
        "min_ev": 1.0,
        "max_picks": 6,
        "stake_per_pick": 100,
    }
    ...
    for row in comparable_rows:
        ...
        result_combo = (
            row.get("result_combo")
            or row.get("actual_combo")
            or row.get("result_trifecta")
            or ""
        )
        payout = (
            row.get("trifecta_payout")
            or row.get("result_payout")
            or row.get("payout")
            or 0
        )
        ...
        shadow_history.append({
            "date": row.get("date") or row.get("race_date"),
            "venue": row.get("venue") or row.get("venue_name"),
            "race_no": row.get("race_no"),
            "fetched_at": shadow.get("fetched_at") or row.get("shadow_fetched_at"),
            "minutes_before_deadline": (
                shadow.get("minutes_before_deadline")
                if shadow.get("minutes_before_deadline") is not None
                else row.get("shadow_minutes_before_deadline")
            ),
            "near_deadline_refresh": bool(shadow.get("near_deadline_refresh")),
            "picks": picks,
            "result_combo": result_combo,
            "payout": payout,
            "hit": str(result_combo) in picked,
        })
...
def shadow_odds_check():
    try:
        ...
        if not race_text or not date_text:
            return jsonify({
                "status": "error",
                "message": "raceとdateを指定してください",
                "example": "/shadow-odds-check?race=蒲郡12R&date=2026-07-17",
            }), 400
        ...
        official_map = {
            str(row.combo): (
                None if pd.isna(row.odds) else float(row.odds)
            )
        ...
        for combo in all_combos:
            ...
            rows.append({
                ...
                "official_current_odds": official_odds,
                "difference": difference,
                "ratio": ratio,
                "saved_EV": ev,
                "mapping_present_both": combo in saved_map and combo in official_map,
                "odds_changed": mismatch,
            })

        rows.sort(
            key=lambda item: (
                item.get("saved_EV") is not None,
                item.get("saved_EV") or -1,
            ),
            reverse=True,
        )
        ...
        return jsonify({
            ...
            "date": date_text,
            "doc_id": doc_id,
            "shadow_fetched_at": shadow.get("fetched_at") or saved.get("shadow_fetched_at"),
            "minutes_before_deadline": shadow.get("minutes_before_deadline") or saved.get("shadow_minutes_before_deadline"),
            "saved_combo_count": len(saved_map),
            "official_combo_count": len(official_map),
            "mapping_complete": len(saved_map) == 120 and len(official_map) == 120,
            "missing_count": missing_count,
            "changed_odds_count": mismatch_count,
            "note": "公式現在オッズは再取得時点の値です。予想時から変動していれば差が出ます。買い目の紐付け確認はcomboと件数を見てください。",
            "top_saved_ev_rows": rows[:30],
        })
...
def run_result_backfill(max_rounds=20):
    ...
    return {
        "status": "ok",
        "unchecked_before": before,
        "unchecked_after": after,
        "recovered": before - after,
        "rounds_run": len(rounds),
        "rounds": rounds,
        "note": (
            "進捗が止まった場合は、残件が未開催・結果未掲載・"
            "取得エラーの可能性があります"
        ),
    }
...
def backfill_results():
    try:
        ...
        return jsonify(
            run_result_backfill(max_rounds=max_rounds)
        )
    except Exception as exc:
        return jsonify({
            "status": "error",
...
def backfill_results_direct():
    try:
        ...
        return jsonify({
            ...
            "unchecked_before": before,
            "unchecked_after": after,
            "recovered_real": before - after,
            "update_result": result,
            "run_again": bool(after > 0 and before > after),
        })
