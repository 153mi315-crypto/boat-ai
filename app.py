...
def allocate_stakes_by_odds(recommendations, budget=1000, unit=100):
    """
    候補ごとの払戻額がなるべく近くなるように、
    1 / odds 比例で100円単位に自動配分する。
    """
    ...
    # 候補数が予算の100円単位を超える場合は上位だけ残す
    max_picks = budget // unit
    recommendations = recommendations[:max_picks]
    count = len(recommendations)

    odds = np.array(
        [max(float(r["odds"]), 1.01) for r in recommendations],
        dtype=float,
    )
    ...
    for rec, stake_units in zip(recommendations, units):
        stake = int(stake_units * unit)
        gross_if_hit = float(rec["odds"]) * stake
        net_if_hit = gross_if_hit - budget

        allocated.append({
            **rec,
            "stake": stake,
            "gross_if_hit": round(gross_if_hit),
            "net_if_hit": round(net_if_hit),
        })
...
def fetch_official_result(date_text, venue_code, race_no):
    ...
    html, url = get_html("raceresult", hd, int(venue_code), int(race_no))
    combo = parse_result_combo(html)
    payout = parse_trifecta_payout(html, combo)
    return combo, payout, url
...
def update_pending_results(limit=100):
    ...
    for doc in docs:
        ...
        if not needs_check and not needs_payout_recheck:
            continue

        checked += 1

        try:
            combo, trifecta_payout, _ = fetch_official_result(
                data["date"],
                data["venue_code"],
                data["race_no"]
            )
...
def build_stats():
    ...
    # 同じ日・会場・Rで重複している過去ログも、
    # created_atが最も新しい予想だけ採用する。
    latest_by_race = {}
    ...
    finished = [r for r in rows if r.get("result_checked") is True]
    ...
    for row in recommended:
        ...
        venue = race_name.rstrip("0123456789R") or "不明"

        stats = venue_map.setdefault(
            venue,
            {
                "venue": venue,
                "predictions": 0,
        ...
        stats["predictions"] += 1
        stats["hits"] += 1 if row.get("hit") is True else 0
        stats["stake"] += int(row.get("stake") or 0)
        stats["payout"] += int(row.get("payout") or 0)

    venue_stats = []

    for stats in venue_map.values():
    ...
    return {
        "saved_predictions": len(rows),
        "finished_races": len(finished),
        "recommended_finished": len(recommended),
        "hit_races": hit_races,
        "hit_rate": (
        ...
        "recent": [
            {
                ...
                "result_predicted_probability": r.get(
                    "result_predicted_probability"
                ),
  ...
  try {
    ...
    let html = `
      <div class="meta">
        ...
        投資予算：${Number(data.budget || 0).toLocaleString()}円<br>
        レース的中見込み：
...
statsBtn.addEventListener("click", async () => {
  ...
  try {
    ...
    if (!res.ok) throw new Error(s.message || s.error || "成績取得失敗");

    const pct = (v) => v == null ? "-" : (Number(v) * 100).toFixed(1) + "%";
    const yen = (v) => Number(v || 0).toLocaleString() + "円";

    let html = `
      <div class="stats-grid">
        <div class="stat"><small>保存予想</small><strong>${s.saved_predictions}</strong></div>
        <div class="stat"><small>結果確認済み</small><strong>${s.finished_races}</strong></div>
        <div class="stat"><small>的中レース</small><strong>${s.hit_races}</strong></div>
        <div class="stat"><small>的中率</small><strong>${pct(s.hit_rate)}</strong></div>
        <div class="stat"><small>収支</small><strong>${yen(s.total_profit)}</strong></div>
        <div class="stat"><small>回収率</small><strong>${pct(s.roi)}</strong></div>
      </div>
      <div class="meta history">
        <b>場ごとの成績</b>
        <div id="venueStats"></div>
      </div>
      <div class="meta history"><b>最近の予想</b>
    `;
    ...
    if (venues.length === 0) {
      venueBox.innerHTML = `
        <div class="empty">確定済みの推奨予想がまだありません</div>
      `;
    } else {
      venueBox.innerHTML = venues.map((v) => `
        <div class="history-row">
          <div class="history-summary">
            <div>
              <b>${esc(v.venue)}</b><br>
              ${Number(v.hits || 0)} / ${Number(v.predictions || 0)}的中
              ・的中率 ${pct(v.hit_rate)}
            </div>
    ...
    (s.recent || []).forEach((r, index) => {
      ...
      if (recs.length === 0) {
        picksHtml = `<div class="history-muted">この予想は見送りでした</div>`;
      } else {
        recs.forEach((pick, i) => {
          picksHtml += `
            <div class="history-pick">
              <b>${i + 1}位 ${esc(pick.combo)}</b>
              <span>${Number(pick.odds || 0).toFixed(1)}倍</span>
              <span>${Number(pick.stake || 0).toLocaleString()}円</span>
            </div>
            <div class="history-muted">
              補正確率 ${(Number(pick.safe_probability || 0) * 100).toFixed(2)}%
              / 保守EV ${Number(pick.safe_EV || 0).toFixed(2)}
            </div>
          `;
        });
      }

      const conditions = (r.special_conditions || []).length
        ? `<div class="special">⚠️ ${esc(r.special_conditions.join(" / "))}</div>`
        : "";

      html += `
        <div class="history-row" data-history-index="${index}">
          <div class="history-summary">
            <div>
              <b>${esc(r.date)} ${esc(r.race)}</b>
              ${r.source === "auto" ? " 🤖" : ""}<br>
              ${esc(r.decision)} / ${hit}
              ${r.profit == null
                ? ""
                : ` / ${Number(r.profit).toLocaleString()}円`}
            </div>
          ...
          <div class="history-detail">
            <div class="history-muted">
              ...
              レース的中見込み：
              <b>${(Number(r.race_hit_probability || 0) * 100).toFixed(1)}%</b>
            </div>
            ${conditions}
            <div style="margin-top:8px">${picksHtml}</div>
            ${
              r.result_combo
                ? `<div style="margin-top:10px">
                    結果：<b>${esc(r.result_combo)}</b>
                    / 払戻 ${Number(r.payout || 0).toLocaleString()}円
                    ${
                      r.result_predicted_probability == null
                        ? `<div class="history-muted">
                            この結果着順の事前予想値は保存されていません
                          </div>`
                        : `<div class="history-detail" style="display:block;margin-top:8px">
                            <b>結果着順の事前評価</b><br>
                            補正確率：
                            ${(Number(r.result_predicted_probability) * 100).toFixed(2)}%<br>
                            予想時オッズ：
                            ${Number(r.result_predicted_odds || 0).toFixed(1)}倍<br>
                            保守EV：
                            ${Number(r.result_predicted_ev || 0).toFixed(2)}<br>
                            予想順位：
                            ${r.result_predicted_rank == null
                              ? "-"
                              : `${Number(r.result_predicted_rank)}位 / 120通り`}<br>
                            推奨対象：
                            ${r.result_was_recommended ? "入っていた" : "入っていなかった"}
                          </div>`
                    }
                  </div>`
                : ""
    ...
    const venueStatsTarget = document.getElementById("venueStats");
    if (venueStatsTarget) {
      venueStatsTarget.replaceWith(venueBox);
    }

    resultBox.querySelectorAll(".history-row").forEach((row) => {
      row.addEventListener("click", () => {
        row.classList.toggle("open");
        const arrow = row.querySelector(".history-arrow");
        if (arrow) {
