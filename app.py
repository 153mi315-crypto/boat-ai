    ...
    for row in recommended:
        ...
        stats = venue_map.setdefault(
            venue,
            {
                ...
                "stake": 0,
                "payout": 0,
                "predicted_hit_probability_sum": 0.0,
                "predicted_hit_probability_count": 0,
            },
        )
        ...
        race_hit_probability = row.get(
            "race_hit_probability"
        )

        if race_hit_probability is None:
            recommendations = row.get(
                "recommendations",
                [],
            )
            race_hit_probability = min(
                1.0,
                sum(
                    float(
                        item.get("safe_probability") or 0
                    )
                    for item in recommendations
                ),
            )

        if race_hit_probability is not None:
            stats[
                "predicted_hit_probability_sum"
            ] += float(race_hit_probability)
            stats[
                "predicted_hit_probability_count"
            ] += 1
