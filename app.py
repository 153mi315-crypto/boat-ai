...
<head>
  ...
  <style>
    ...
    * { box-sizing: border-box; }
    ...
    .history-row { border-bottom:1px solid var(--line); padding:10px 0; font-size:14px; }
    .foot {
      color: var(--muted);
      font-size: 13px;
      line-height: 1.6;
...
statsBtn.addEventListener("click", async () => {
  ...
  try {
    ...
    (s.recent || []).forEach((r) => {
      const hit = r.hit === true ? "🎯 的中" : (r.hit === false ? "はずれ" : "未確定");
      html += `
        <div class="history-row">
          ${esc(r.date)} ${esc(r.race)} / ${esc(r.decision)} / ${hit}
          ${r.profit == null ? "" : ` / ${Number(r.profit).toLocaleString()}円`}
        </div>
      `;
    });
