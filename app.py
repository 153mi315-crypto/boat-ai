...
def build_roi_analysis():
    ...
    for doc in docs:
        row = doc.to_dict() or {}
        row['id'] = doc.id
        if row.get('result_checked') is True and row.get('result_combo'):
            rows.append(row)

    def normalize_combo(value):
        digits = [c for c in str(value or '') if c.isdigit()]
        return '-'.join(digits[:3]) if len(digits) >= 3 else str(value or '')
    ...
    for model in ('old', 'new'):
        ...
        total_payout = 0
        for row in rows:
            result_combo = normalize_combo(row.get('result_combo'))
            trifecta_payout = int(row.get('trifecta_payout') or 0)
        ...
        output[model] = {'summary': {'races': len(rows), 'bets': total_bets, 'hits': total_hits, 'stake': total_stake, 'payout': total_payout, 'profit': total_payout - total_stake, 'roi': total_payout / total_stake if total_stake else None}, 'groups': {key: finalize(value) for key, value in groups.items()}}
    return {'status': 'ok', 'finished_races': len(rows), 'stake_rule': '各買い目100円の単純比較', 'analysis': output}
